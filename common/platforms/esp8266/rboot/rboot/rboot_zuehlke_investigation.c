...
/******************* HACK START, sets Pin 5 as output**************************/

	int IO_Pin_NR = 5; //IO5 on ESP

	PIN_FUNC_SELECT(PERIPHS_IO_MUX_GPIO5_U, FUNC_GPIO5); //CHANGE THIS ALSO IF YOU CHANGE IO_PIN_NR

	GPIO_REG_WRITE(GPIO_ENABLE_W1TS_ADDRESS, BIT(IO_Pin_NR));

  unsigned int pin_reg = GPIO_REG_READ(GPIO_PIN_ADDR(GPIO_ID_PIN(IO_Pin_NR)));
  pin_reg &= (~0x380);
  pin_reg |= (GPIO_PIN_INTR_DISABLE << 7);
  GPIO_REG_WRITE(GPIO_PIN_ADDR(GPIO_ID_PIN(IO_Pin_NR)), pin_reg);

	GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS, BIT(IO_Pin_NR));

	/* Pin driver -> direct drive, pad driver -> open drain. */
	unsigned int pin_addr = GPIO_REG_READ(GPIO_PIN_ADDR(GPIO_ID_PIN(IO_Pin_NR)));
	pin_addr &= ~GPIO_PIN_PAD_DRIVER_MASK;
	GPIO_REG_WRITE(GPIO_PIN_ADDR(GPIO_ID_PIN(IO_Pin_NR)), pin_addr);

/******************* HACK END ************************************/

	ets_printf("\r\nrBoot v1.2.1-ZInvestigations - richardaburton@gmail.com\r\n");
	...
	if (updateConfig) {
		WRITE_PERI_REG(PERIPHS_GPIO_BASEADDR + GPIO_OUT_W1TC_ADDRESS, 1 << IO_Pin_NR);	//time meas: set Pin 5 LOW
		...
		SPIEraseSector(BOOT_CONFIG_SECTOR);
		SPIWrite(BOOT_CONFIG_SECTOR * SECTOR_SIZE, buffer, SECTOR_SIZE);
		WRITE_PERI_REG(PERIPHS_GPIO_BASEADDR + GPIO_OUT_W1TS_ADDRESS, 1 << IO_Pin_NR);		//time meas end: set Pin 5 HIGH
	}
